...
public class CustomProcessDiagramGenerator {
    ...
    private static List<String> boundaryType = Arrays.asList(new String[]{"boundaryTimer","boundaryMessage","boundarySignal"});
    ...
    protected static void init() {
        ...
        //add
        eventType.add("boundaryTimer");
        eventType.add("boundaryMessage");
        eventType.add("boundarySignal");
...
//        //注释了下面的代码，防止流程图和xml一起部署，坐标偏移的问题	
...
//                        historicActivityInstanceQueryImpl, page);
        //update by lipw 2014-10-30
        //开启新事务，查询最新的数据
        ActivitiHelpService activitiHelpService = (ActivitiHelpService)SpringTools.getBean(ActivitiHelpService.class);
        
        List<HistoricActivityInstance> activityInstances = activitiHelpService.queryHisActInst4NewTransaction(processInstanceId); 
      //update by lipw end
    ...
    public void drawHistoryFlow(BufferedImage image, String processInstanceId) {
        ...
        for (Edge edge : graph.getEdges()) {
        	if (!boundaryType.contains(edge.getType())){
