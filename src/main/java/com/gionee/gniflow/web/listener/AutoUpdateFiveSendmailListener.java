...
import com.gionee.gniflow.web.util.TestEmail;

public class AutoUpdateFiveSendmailListener implements ExecutionListener{
      
	private static final long serialVersionUID = 1L;
	private Logger logger = LoggerFactory
			.getLogger(AutoUpdateFiveSendmailListener.class);
	private ProcessHelpService processHelpService;
	private UserDao userDao;
	@Override
	public void notify(DelegateExecution execution) throws Exception {
		// TODO Auto-generated method stub
				logger.debug("Running in AutoUpdateFiveSendmailListener begin!");
			...
			userDao = (UserDao) SpringTools.getBean(UserDao.class);
				...
				String SqaDismassAffirmAudi=(String) runtimeService.getVariable(execution.getId(), "SqaDismassAffirmAudi");
				if(SqaDismassAffirmAudi.equals("1")){
				// 软件名称
				...
				// 应用升级管理编号
				String AppUpgradeManageNumber = (String) runtimeService.getVariable(
						execution.getId(), "AppUpgradeManageNumber");
				...
				// 风险
				String risk = (String) runtimeService.getVariable(execution.getId(),
						"risk");
				...
				// 备注
				String remark = (String) runtimeService.getVariable(execution.getId(),
						"remark");
				...
				String SqaAudi = (String) runtimeService.getVariable(execution.getId(),
						"SqaAudi");
				SqaAudi = SqaAudi.equals("1") ? "通过" : "不通过";
				...
				/*// 反馈人
				String feedbackAudiTwoName = (String) runtimeService.getVariable(
						execution.getId(), "feedbackAudiTwoName");		
				
				// 反馈时间
				String feedbackAudiTwoSignDate = (String) runtimeService.getVariable(
						execution.getId(), "feedbackAudiTwoSignDate");	
				
				// 用户反馈系统
				String userFeedbackTwoAudiAdvice = (String) runtimeService.getVariable(
						execution.getId(), "userFeedbackTwoAudiAdvice");	
				
				// 售后
				String afterTwoService = (String) runtimeService.getVariable(
						execution.getId(), "afterTwoService");
				
				// 论坛
				String feedbackAudiTwoAdvice = (String) runtimeService.getVariable(
						execution.getId(), "feedbackAudiTwoAdvice");
				
				// 报错
				String errorTwo = (String) runtimeService.getVariable(
						execution.getId(), "errorTwo");*/
				
				// SQA审批人
				String SqaDismassAffirmName = (String) runtimeService.getVariable(
						execution.getId(), "SqaDismassAffirmName");
				
				// SQA审批时间
				String SqaDismassAffirmSignDate = (String) runtimeService.getVariable(
						execution.getId(), "SqaDismassAffirmSignDate");
				
				// 处理意见
				String SqaDismassAffirmAdvice = (String) runtimeService.getVariable(
						execution.getId(), "SqaDismassAffirmAdvice");
				
				String titel = "APK自升级流程《"+softwareName+"》" + 
						versionNumber + "适配机型"
						+ equipmentType + "正式发布不通过";
				String table="";
				 //修改表单
			    ...
			    +"<tr ><td  colspan=4 align=center><font size=5px color=red>SQA评审正式发布是否正常</font></td></tr>"
			    +"<tr><td colspan=2>审核人:"+SqaDismassAffirmName+"</td><td colspan=2>审核时间 :"+SqaDismassAffirmSignDate+"</td></tr>"
			    +"<tr><td colspan=2>评审结果:不正常</td><td colspan=2>备注:"+SqaDismassAffirmAdvice.replaceAll("\r\n", "<br/>")+"</td></tr></table></br>");
				
				/*content.append("<table width=100%  border=1 align=center cellpadding=0 cellspacing=0>"
							+"<tr ><td  colspan=4 align=center><font size=5px color=red>正式发布用户反馈专员结果确认</font></td></tr>"
							+"<tr><td colspan=2>反馈人:"+feedbackAudiTwoName+"</td><td colspan=2>反馈时间:"+feedbackAudiTwoSignDate+"</td></tr>"
							+"<tr><td colspan=2>用户反馈系统:"+userFeedbackTwoAudiAdvice+"</td><td colspan=2>售后:"+afterTwoService+"</td></tr>"
							+"<tr><td colspan=2>论坛:"+feedbackAudiTwoAdvice+"</td><td colspan=2>报错:"+errorTwo+"</td></tr></table></br>");*/
				...
				content.append("<table width=100%  border=1 align=center cellpadding=0 cellspacing=0>"
					    ...
					    +"<tr><td colspan=2>评审结果:通过</td><td colspan=2>备注:"+operationManagerTwoAdvice.replaceAll("\r\n", "<br/>")+"</td></tr></table></br>");
			...
			/*	content.append("<table width=100%  border=1 align=center cellpadding=0 cellspacing=0>"
				...
				content.append("<table width=100%  border=1 align=center cellpadding=0 cellspacing=0>"
						...
						+"<tr><td colspan=2>评审结果:通过</td><td colspan=2>备注:"+operationManagerAdvice.replaceAll("\r\n", "<br/>")+"</td></tr></table></br>");
				...
				content.append("<table width=100%  border=1 align=center cellpadding=0 cellspacing=0>"
						...
						+"<tr><td colspan=2>评审结果:通过</td><td colspan=2>备注:"+SqaAdvice.replaceAll("\r\n", "<br/>")+"</td></tr></table></br>");
				...
				if(!StringUtils.isEmpty(applyUserId)){
				  processHelpService
				  .sendEmail4Orgs(processHelpService.getOrgId(applyUserId), titel,
				  content.toString()); 
				  }
				  //设置固定收件人邮箱 
				String[] Emails={"dujq@gionee.com","zhuying@gionee.com","xiangmj@gionee.com"
				  ,"caoht@gionee.com","shijy<shijy@gionee.com>","liaojh@gionee.com",
				  "sw_user@gionee.com","znyf_testleader@gionee.com"
