...
public class AutoUpdateTwoSendmailListener implements ExecutionListener{

	private static final long serialVersionUID = 1L;
	private Logger logger = LoggerFactory.getLogger(AutoUpdateTwoSendmailListener.class);
	private ProcessHelpService processHelpService;
	private UserDao userDao;
	@Override
	public void notify(DelegateExecution execution) throws Exception {
		// TODO Auto-generated method stub
		logger.debug("Running in AutoUpdateTwoSendmailListener begin!");
	  ...
	  //测试结果
	    ...
	    //SQA审批人 
	    String SQAName = (String)runtimeService.getVariable(execution.getId(), "SQAName");
	    
	    //SQA审批时间 
	    String SQASignDate = (String)runtimeService.getVariable(execution.getId(), "SQASignDate");
	    
	    //是否通过
	    String SqaAudi = (String)runtimeService.getVariable(execution.getId(), "SqaAudi");
	    SqaAudi=SqaAudi.equals("0")?"通过":"不通过";
	    
	    //处理意见
	    String SqaAdvice = (String)runtimeService.getVariable(execution.getId(), "SqaAdvice");
	    
	    //运营经理评审人
	    String OperationManagerName = (String)runtimeService.getVariable(execution.getId(), "OperationManagerName");
	    
	    //运营经理审批时间 
	    String OperationManagerSignDate = (String)runtimeService.getVariable(execution.getId(), "OperationManagerSignDate");
	   
	    //处理意见
	    String operationManagerAdvice = (String)runtimeService.getVariable(execution.getId(), "operationManagerAdvice");
	    
	    
		String titel="APK自升级流程 《"+softwareName+"》"+
		versionNumber+"适配机型"+equipmentType+"预发布通过";
		/*String content="运营经理审批:     运营经理评审人:"+OperationManagerName+"         运营经理审批时间 :"+OperationManagerSignDate+"      是否通过:通过"+
		"     处理意见:"+operationManagerAudi+
		"SQA审批:     SQA审批人 :"+SQAName+"         SQA审批时间  :"+SQASignDate+"      是否通过:通过"+
		"     处理意见:"+SqaAdvice
		+"     开始表单           "+"应用升级管理方式:"+AppUpgradeManage+"            应用升级管理编号:"+AppUpgradeManageNumber
